package org.example.proxy.coupon.service.impl;
...
import org.example.proxy.coupon.mapper.CouponInfoMapper;
import org.example.proxy.coupon.mapper.CustomerCouponMapper;
import org.example.proxy.coupon.service.CouponInfoService;
import org.example.proxy.model.entity.coupon.CouponInfo;
import org.example.proxy.model.entity.coupon.CustomerCoupon;
...
public class CouponInfoServiceImpl extends ServiceImpl<CouponInfoMapper, CouponInfo> implements CouponInfoService {
    ...
    public Boolean receive(Long customerId, Long couponId) {
        ...
        if(couponInfo.getExpireTime().before(new Date())) {
            throw new ProxyException(ResultCodeEnum.COUPON_EXPIRE);
        }

        //3 检查库存，发行数量 和 领取数量
        if(couponInfo.getPublishCount() != 0 &&
                couponInfo.getReceiveCount() == couponInfo.getPublishCount()) {
            throw new ProxyException(ResultCodeEnum.COUPON_LESS);
        }
        RLock lock = null;
        try {
            ...
            if(flag) {
                ...
                if(couponInfo.getPerLimit() > 0) {
                    ...
                    if(count >= couponInfo.getPerLimit()) {
                        throw new ProxyException(ResultCodeEnum.COUPON_USER_LIMIT);
                    }
                }
