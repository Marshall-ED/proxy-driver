...
import org.example.proxy.driver.service.DriverService;
...
public class DriverServiceImpl implements DriverService {
    ...
    public Boolean startService(Long driverId) {
        ...
        if(driverLoginVo.getAuthStatus()!=2) {
            throw new ProxyException(ResultCodeEnum.AUTH_ERROR);
        }

        //2 判断当日是否人脸识别
        Boolean isFace = driverInfoFeignClient.isFaceRecognition(driverId).getData();
        if(!isFace) {
            throw new ProxyException(ResultCodeEnum.FACE_ERROR);
        }
